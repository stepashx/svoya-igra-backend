 ...
 * Captain-only. When the count of ready teams first reaches `minTeamsToStart`
 * the room advances TEAM_SETUP → READY_CHECK. Also broadcasts
 * `game-can-start-changed` with the current "host can start" flag.
 *
 * Runs in a transaction guarded by a per-room advisory lock (as CreateTeam /
 * JoinTeam / StartGame): two captains toggling ready at once would otherwise
 * race on the ready-count read and the threshold transition. The lock serialises
 * them so the count is consistent and TEAM_SETUP → READY_CHECK fires exactly
 * once. The stage canon (READY_CHECK at ≥ MIN ready) is unchanged — the lock
 * only makes the transition reliable.
 */
@Injectable()
