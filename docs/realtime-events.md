...
| `server:gameplay:answer-accepted` | server | gameplay | room | Ведущий принял ответ — итог проверки, НЕ начисление | §16.4 | Эмитируется с 6.2a из ReviewAnswerUseCase (ведущий `POST questions/review`) при принятии; payload `{ roomId, cellId, teamId: string \| null }` (`teamId` = открывающая команда) |
| `server:gameplay:answer-rejected` | server | gameplay | room | Ведущий отклонил ответ — итог проверки, НЕ начисление | §16.4 | Эмитируется с 6.2a из ReviewAnswerUseCase при отклонении; ТА ЖЕ позиция/форма эмиссии, что у `answer-accepted` (флаг принятия выбирает имя); payload `{ roomId, cellId, teamId: string \| null }` |
...
| `server:gameplay:score-changed` | server | gameplay | room | Изменились очки команды | §16.4 | Эмитируется с 7.1 из ReviewAnswerUseCase (принятая проверка, ПОЛОЖИТЕЛЬНАЯ `delta`); ТАКЖЕ с 8.3 из PurchaseItemUseCase при списании в магазине, ОТРИЦАТЕЛЬНАЯ `delta` (двигается только `balance`, `earnedScore` держится); payload `{ roomId, teamId, earnedScore, balance, delta }` |
