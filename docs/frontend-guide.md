...
| `GAME_BOARD` | Поле 6×5, активная команда | Выбрать ячейку (капитан активной команды); открыть/отклонить (ведущий) | Старт игры или возврат из проверки/магазина → `QUESTION_OPENED`, когда **ведущий открывает** |
...
- **Развилка A — `ANSWER_REVIEW` → `GAME_BOARD` | `SHOP`.** После того как ведущий проверяет
  ...
  - При входе в магазин блок проверки заканчивается на `shop-final-opened`, если поле
...
всем в комнате `question-timer-ended`, перемещая комнату в `ANSWER_REVIEW` (затем проверка
...
`shop-final-opened` приходят **последними** в блоке проверки [§4.2](#42--бой).
