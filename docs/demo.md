...
Docker ([local-development → run mode B](local-development.md#режим-запуска-b--бэкенд-на-хосте-инфраструктура-в-docker)):
...
- **Одна непрерывная сессия.** Присутствие сокетов и таймеры ответа / магазина /
  ...
  [README → Known limitations](../README.md#известные-ограничения)). Они теряются при
...
- **QR-ассеты — заглушки.** Шесть засиженных QR-инструментов — это заглушки
...
засиживаются сидами** — они создаются живой игрой. С чего вы начинаете:
...
каждого review**, принятого или отклонённого.
...
4. **Ведущий проводит review** — `POST /api/rooms/:code/questions/review`
...
**Развилка магазина.** После review комната входит в **магазин**, если число
...
`shop-final-opened`, на исчерпанном поле) приходит последним в блоке review.
...
3. **Последний докладчик** — завершение финальной команды запускает
   ...
   полностью **derived** — `GET /api/rooms/:code/defense/state` пересчитывает его,
...
[README → Troubleshooting](../README.md#устранение-неполадок); относящиеся к демо:
...
- **Порт уже занят (`3000`, `5432`, `9000`, `9001`).** Другой процесс или старый
  ...
  [`.env`](../.env.example). См. [README → Troubleshooting](../README.md#устранение-неполадок).
